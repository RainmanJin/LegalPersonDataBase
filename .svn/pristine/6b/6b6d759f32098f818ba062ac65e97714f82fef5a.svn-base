            ...
            series: [
                {
                    ...
                    color: ['#5b9bd5', '#ed7d31', '#636363', '#ffc000', '#a6a6a6', '#70ad47', '#2e75b6', '#ff5151', '#8a48c8'],
                    label: {
                        normal: {
                            position: 'inner',
                            formatter: '{d}%'
...
//                            formatter:"{b} {d}%",
    ...
    function legalPersonNumber() {
        httpRequest({
            ...
            success: function (data) {
                ...
                var lp = $(".legalperson");
                lp.text(energy+"条");
                //var data = data.data.DimTotal.toString();
     ...
     * 业务数据沉淀量分类统计
     */
    function StatisticHandler(id) {
        httpRequest({
            url: "/data/datacount/DimTotal.json?id=" + id,
            success: function (data) {
            	var DimTotal = $(".id"+id.toString());
        		var data = data.data.DimTotal;
        		DimTotal.text(data + "条");
            },
            error: function (jqXHR) {
                alert("出错");
            }
    ...
    for(var i = 1; i<10; i++) StatisticHandler(i);
    legalPersonNumber();
    departTop();
    departWd();
    getNowFormatDate();
