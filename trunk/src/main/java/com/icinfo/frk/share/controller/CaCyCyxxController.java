...
import com.icinfo.frk.share.service.ICaCyCyxxService;
...
public class CaCyCyxxController extends BaseController {
     ...
     * @throws Exception
     */
    @RequestMapping(value = "doGetCyxxList.json", method = RequestMethod.GET, produces = "application/json;charset=UTF-8")
    @ResponseBody
    public ApiResponse doGetCyxxList(@RequestParam(name = "pageNum", required = true) int pageNum,
                                     @RequestParam(name = "pageSize", required = true) int pageSize) throws Exception {
        Map<String, Object> map = new HashMap<>();
        List<CaCyCyxx> list = caCyCyxxService.doGetCyxxList(map, pageNum, pageSize);
